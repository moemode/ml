...
def hinge_loss_single(
    feature_vector: np.ndarray, label: float, theta: np.ndarray, theta_0: float
) -> float:
    """
...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    """
    Z = labels * (feature_matrix @ theta + theta_0)
    return np.average(np.maximum(0, 1 - Z))
...
def average_perceptron(feature_matrix, labels, T):
    ...
    for _ in range(T):
        for i in get_order(nsamples):
            ...
            theta_sum += theta
            theta_0_sum += theta_0
    return theta_sum / (T*nsamples), theta_0_sum / (T*nsamples)
...
def pegasos_single_step_update(feature_vector, label, L, eta, theta, theta_0):
    ...
    theta = (1-eta*L) * theta
    # no scaling for theta_0
    if z <= 1:
        theta += eta * label * feature_vector
        theta_0 += eta * label
    return theta, theta_0
...
def pegasos(feature_matrix, labels, T, L):
    ...
    for _ in range(T):
        for i in get_order(nsamples):
            theta, theta_0 = pegasos_single_step_update(feature_matrix[i], labels[i], L, 1/np.sqrt(iterno), theta, theta_0)
            iterno += 1
    return theta, theta_0
...
def extract_bow_feature_vectors(reviews, indices_by_word, binarize=True):
    ...
    if binarize:
        feature_matrix = np.minimum(feature_matrix, 1)
